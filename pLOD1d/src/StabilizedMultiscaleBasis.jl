...
using pLOD1d.LegendrePolynomials: assemble_rectangular_matrix, assemble_legendre_mass_matrix
...
"""
function stabilized_multiscale_bases(aₕ::Function, V::FESpace, domain::SVector{2, T}, n::Int, N::Int, l::Int, p::Int; strategy=DHM25(), show_progress=true) where T<:Real
  ...
  @showprogress enabled=show_progress "Computing stabilized-pLOD bases with [$(strategy |> typeof |> nameof)]" for K = 1:N
    sol = zeros(T, n+1)   
    ...
    elseif(strategy==HLM25()) 
      elem_coarse = elements_in_coarse_scale_patch(1:N, N, 0)      
      D = (Z - C)*H;      
